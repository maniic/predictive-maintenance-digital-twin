      ...
      <div className="flex-1 flex relative">
        ...
        >
          ...
          <div className="pt-4 border-t border-[var(--border)] space-y-2">
            {simLoading ? (
              <button
                disabled
                className="btn-primary w-full opacity-50 flex items-center justify-center gap-2"
              >
                <Spinner />
                Computing...
              </button>
              ...
              <button
                ...
                disabled={isComplete && trajectory}
              >
        ...
        <main className="flex-1 p-4 md:p-6 overflow-auto">
          <div className="max-w-4xl">
            ...
            <DemoNotice>
              This degradation curve is computed in your browser, not by the digital-twin simulator
              or a trained model.
            </DemoNotice>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-px border border-[var(--border)] mb-5">
              {[
                ...
                {
                  label: 'Health',
                  value: currentPoint ? `${(currentPoint.health_score * 100).toFixed(0)}%` : '—',
                },
              ].map((m) => (
                <div key={m.label} className="bg-[var(--bg-surface)] p-4">
                  <div className="data-label mb-1">{m.label}</div>
                  <div
                    className="data-value text-xl md:text-2xl"
                    style={m.color ? { color: m.color } : {}}
                  >
                    {m.value}
                  </div>
                  {m.sub && (
                    <div className="font-mono text-[0.6rem] text-[var(--text-faint)]">{m.sub}</div>
                  )}
                </div>
              ))}
            </div>
