...
class CMAPSSDataLoader:
    ...
    def _load_txt_file(self, file_path: Path) -> pd.DataFrame:
        ...
        if df.shape[1] != len(self.COLUMN_NAMES):
            raise ValueError(
                f"Expected {len(self.COLUMN_NAMES)} columns in {file_path.name}, got {df.shape[1]}"
            )
    ...
    def load_dataset(self, dataset_name: str) -> CMAPSSDataset:
        ...
        if dataset_name not in self.DATASET_INFO:
            raise ValueError(
                f"Unknown dataset: {dataset_name}. Must be one of {list(self.DATASET_INFO.keys())}"
            )
        ...
        if len(rul) != n_test_engines:
            raise ValueError(
                f"RUL file has {len(rul)} entries but test data has {n_test_engines} engines"
            )
